...
if rg '^[[:space:]]*(command|script):.*powerprofilesctl' "$ROOT/shell/plugins/services/battery/Service.qml" "$ROOT/shell/plugins/menu/Menu.qml" >/dev/null; then
  fail "shell plugins do not call powerprofilesctl directly"
fi
pass "shell plugins avoid direct powerprofilesctl calls"
